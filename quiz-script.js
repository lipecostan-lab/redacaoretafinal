class PersuasiveQuiz {
    ...
    createPersonalizedMessage() {
        ...
        if (level === 'beginner') {
            ...
            message.subheadline = 'Método comprovado para iniciantes saírem do ZERO e alcançarem 800+ pontos na redação ENEM';
        } else if (level === 'intermediate') {
            message.headline = 'Pare de Ficar na MÉDIA - Chegou Sua Hora!';
            message.subheadline = 'Sistema definitivo para quebrar a barreira dos 700 pontos e garantir sua vaga na universidade';
        } else {
            message.headline = 'O Último Passo Para a NOTA MÁXIMA!';
            message.subheadline = 'Técnicas avançadas para perfeccionistas que querem os 900+ pontos em medicina/direito';
        }
        
        // Mensagens de urgência personalizadas
        if (urgency === 'high') {
            message.urgencyMessage = '⚠️ ATENÇÃO: Com o ENEM se aproximando, cada dia perdido é uma oportunidade desperdiçada!';
        } else if (urgency === 'medium') {
            message.urgencyMessage = '📅 Você tem uma vantagem: ainda há tempo para se preparar adequadamente!';
        } else {
            message.urgencyMessage = '🎯 Começar cedo é o segredo dos maiores sucessos no ENEM!';
        }
        
        // Foco na solução baseado na dificuldade principal
        const solutionMap = {
            'structure': 'Método exclusivo de estruturação de parágrafos que elimina a confusão',
            'arguments': 'Banco de argumentos premium com exemplos que impressionam os corretores',
            'proposal': 'Fórmula infalível para propostas de intervenção que garantem nota máxima',
            'time': 'Técnica de gestão de tempo que te faz terminar a redação em 45 minutos'
        };
        
        message.solutionFocus = solutionMap[mainDifficulty] || 'Sistema completo que resolve todas as dificuldades na redação';
